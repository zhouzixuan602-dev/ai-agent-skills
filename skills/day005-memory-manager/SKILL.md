...
**痛点**: 长对话超出上下文窗口，历史信息丢失，Agent "失忆"

---

## 问题描述

Agent 在多轮对话中会积累大量历史消息，当 token 数逼近模型上下文限制（如 128K）时，开发者通常直接截断最早的消息。这导致：

- 关键早期信息（用户偏好、任务背景）被丢弃
- Agent 对早期决策"失忆"，开始重复问题或给出矛盾答案
- 用户体验极差，必须重新解释已讲过的内容

真实场景：客服 Agent 经过 50 轮对话后忘记用户一开始说的订单号；代码助手忘记项目的技术栈约束。
...
新消息
  ...
  ▼
┌─────────────────────────────────────┐
│  Working Memory (最近 N 轮，完整保留)  │  ← 短期记忆，精确
└─────────────────────────────────────┘
  │ 超出阈值时压缩
  ▼
┌─────────────────────────────────────┐
│  Summary Memory (压缩摘要，1-2段)    │  ← 中期记忆，概要
└─────────────────────────────────────┘
  │ 提取关键实体
  ▼
┌─────────────────────────────────────┐
│  Entity Memory (关键实体字典)         │  ← 长期记忆，结构化
│  {"user_name": "Alice", "order": ..}│
└─────────────────────────────────────┘
```

构建 context 时：Entity Memory + Summary Memory + Working Memory 依次拼接，总 token 可控。
...
| 指标 | 简单截断（改进前） | 三层记忆（改进后） |
|------|-----------------|-----------------|
| 上下文 token 用量 | 不可控，趋近上限 | 稳定在阈值以下 |
| 早期关键信息保留 | ❌ 丢失 | ✅ 结构化保留 |
| 用户需要重复自己 | 频繁 | 极少 |
| 实现复杂度 | 低 | 中（约 120 行）|
| 压缩调用额外成本 | 无 | 每次压缩 ~100 token（haiku 极廉价）|
...
---
*Day 005 · AI Agent Skills Daily · Melbourne, Australia*
